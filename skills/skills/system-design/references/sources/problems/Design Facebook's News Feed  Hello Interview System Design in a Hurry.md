---
title: "Design Facebook's News Feed | Hello Interview System Design in a Hurry"
...
To do this, we'll have a simple table using the entire relation with userFollowing as the partition key and userFollowed as the sort key. We can also create a Global Secondary Index (GSI) with the reverse relationship (e.g. partition key of userFollowed and sort key of userFollowing) to allow us to look up all the followers of a given user.
...
That seems to work. For a given user, we first request all of the users they follow from the table. Then we request all the posts from those N users from the Post table via its GSI. Then we sort all of those posts by timestamp and return the results to the user!
...
For users following a large number of users we can keep a PrecomputedFeed table. Instead of querying the and Post tables, we'll be able to pull from this precomputed table. Then, when a new post is created, we'll simply add to the relevant feeds.
...
<audio src="https://b1be528ac540eb1c77e93beeed000a6c.r2.cloudflarestorage.com/hellointerview-files/private-content/narration/v1/4ab71a0cf1a9b80baa46c8e9bc4f2682/c0e25d4330d039250c2956dbea149749/aura-2-thalia-en/3eceb87a6500da99fc0418230d25645e/chunk-0.mp3?X-Amz-Algorithm=AWS4-HMAC-SHA256&amp;X-Amz-Content-Sha256=UNSIGNED-PAYLOAD&amp;X-Amz-Credential=95ecb0a901e86377617caf7f3477127c%2F20260725%2Fauto%2Fs3%2Faws4_request&amp;X-Amz-Date=20260725T210319Z&amp;X-Amz-Expires=172800&amp;X-Amz-Signature=4eda1367609bf0c6660a8780929c407979526291069052fded1cd6725ae6da98&amp;X-Amz-SignedHeaders=host&amp;x-amz-checksum-mode=ENABLED&amp;x-id=GetObject"></audio>